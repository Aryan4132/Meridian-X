...
async def capture_and_analyze_screen():
  """
  Captures a full screenshot, sends it to the local Ollama vision model (e.g. moondream:1.8b),
  and broadcasts a proactive nudge with the analysis results.
  """
  # BUG-59 fix: use tempfile.mktemp with unique prefix to prevent concurrent calls from
  # overwriting the same CWD-relative "temp_screenshot.png" file.
  import tempfile
  ...
  try:
    ...
    # BUG-59 fix: always clean up the temp file, even if the Ollama call fails.
    try:
